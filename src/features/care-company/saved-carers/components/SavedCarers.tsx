...
import { MapPin, Star, BookmarkCheck, MessageSquare, Loader2 } from "lucide-react";
import { useSavedCarers } from "../hooks/useSavedCarers";

export default function SavedCarers() {
  const { carers, removeCarer, isLoading } = useSavedCarers();
          ...
          <div className="p-4 sm:p-6 lg:p-8">
            {isLoading ? (
              ...
              <div className="flex min-h-[300px] flex-col items-center justify-center rounded-2xl bg-white p-12 text-center border border-slate-100">
                <BookmarkCheck className="h-12 w-12 text-slate-300 mb-3" />
                <h3 className="text-lg font-semibold text-slate-700">No Saved Carers Yet</h3>
                <p className="text-sm text-slate-500 mt-1">Carers you bookmark will appear here.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {carers.map((carer) => {
                  const carerId = carer.carerId || carer.id || carer._id || "1";
                  return (
                    <div
                      key={carerId}
                      className="overflow-hidden rounded-2xl bg-white shadow-[0px_4px_6px_0px_rgba(0,0,0,0.06)] border border-neutral-100 flex flex-col transition-all hover:shadow-lg group relative"
                    >
                      {/* Image Section with Available Badge & Unsave Button */}
                      <div className="relative h-64 w-full overflow-hidden bg-neutral-100 block">
                        <Link href={`/care-company/save-carers/${carerId}`}>
                          <img
                            src={carer.image}
                            alt={carer.name}
                        ...
                        {carer.available && (
                          <div className="absolute left-4 top-4 rounded-[45px] bg-white px-3.5 py-1.5 shadow-sm flex items-center justify-center pointer-events-none">
                        ...
                        <button
                          onClick={() => removeCarer(carerId)}
                          title="Remove from saved"
                          className="absolute right-4 top-4 h-9 w-9 rounded-full bg-white/90 shadow-md flex items-center justify-center text-[#2b6ea6] hover:text-red-500 hover:bg-white transition-colors"
                        >
                          <BookmarkCheck className="h-5 w-5 fill-[#2b6ea6]" />
                        </button>
                      </div>
                      ...
                      <div className="p-6 flex flex-col justify-between flex-1 gap-6">
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            <Link
                              href={`/care-company/save-carers/${carerId}`}
